...
def download_and_extract_csv(url, local_zip_path, local_extract_path):
    ...
    local_zip_dir = spark._jvm.java.io.File(os.path.dirname(local_zip_path))
...
try:
    ...
    for root, dirs, files in spark._jvm.java.io.File(local_extract_path).walk():
        for file in files:
            spark._jvm.java.io.File(os.path.join(root, file)).delete()
